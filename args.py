...
restore_path = './data/darknet_weights/yolov3.ckpt'  # 要恢复的权重路径，可以用yolov3作初始权重.
...
anchor_path = './data/pos_anchors.txt'  # The path of the anchor txt file.
class_name_path = './data/my_data/pos.names'  # The path of the class names.
...
letterbox_resize = True  # Whether to use the letterbox resize, i.e., keep the original aspect ratio in the resized image. 是否使用letterbox调整大小，即在调整大小的图像中保留原始宽高比
total_epoches = 20  # 总批次，原来100
train_evaluation_step = 100  # Evaluate on the training batch after some steps.经过一些步骤，评估培训批次。
val_evaluation_epoch = 2  # Evaluate on the whole validation dataset after some epochs. Set to None to evaluate every epoch.经过一段时间后，对整个验证数据集进行评估。设置为None以评估每个时期。
save_epoch = 5  # Save the model after some epochs.每5批保存一次模型
batch_norm_decay = 0.99  # decay in bn ops原来是0.99
weight_decay = 5e-4  # l2 weight decay
global_step = 0  # used when resuming training

### tf.data parameters
num_threads = 10  # Number of threads for image processing used in tf.data pipeline.使用的线程数，原始是10
prefetech_buffer = 5  # Prefetech_buffer used in tf.data pipeline.

### Learning rate and optimizer
optimizer_name = 'momentum'  # Chosen from [sgd, momentum, adam, rmsprop]优化器
...
lr_lower_bound = 1e-6  # The minimum learning rate.最低学习率。
